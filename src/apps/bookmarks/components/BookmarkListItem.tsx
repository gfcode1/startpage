...
export default function BookmarkListItem({
  ...
  return (
    ...
    >
      {bookmark.favicon ? (
        <img src={bookmark.favicon} alt="" width={16} height={16} style={{ flexShrink: 0 }} />
      ) : (
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <Group gap={6} align="center">
          ...
          {bookmark.isFavorite && <Icon icon="lucide:star" width={10} color="var(--mantine-color-yellow-5)" />}
          {bookmark.isReadLater && <Icon icon="lucide:bookmark-plus" width={10} color="var(--mantine-color-blue-5)" />}
        </Group>
        <Text size="xs" c="dimmed" lineClamp={1}>{bookmark.url.replace(/^https?:\/\//, '')}</Text>
      </div>
